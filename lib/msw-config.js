...
export default class MswConfig {
  ...
  create(mirageServer, mirageConfig) {
    ...
    this.msw = this._create(mirageServer, mirageConfig);
    ...
    [
      ...
      this[verb] = (path, ...args) => {
        ...
        this.msw.use(
          rest[verb](fullPath, async (req, res, ctx) => {
            let queryParams = {};
            ...
            return res(ctx.status(code), (res) => {
              res.body = response;
  ...
  _create(mirageServer, mirageConfig) {
    let msw = setupWorker();

    let logging = mirageConfig.logging || false;
    msw.start({
      quiet: !logging,
    });
  ...
  start() {
    // unneeded for msw implementation
  }
